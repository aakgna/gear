...
export default function DiscussionScreen() {
  ...
  useEffect(() => {
    (async () => {
      try {
        await logScreenView("Discussion");
      } catch (error) {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      try {
        await logCommentPosted(currentQuestionId, "top_level");
      } catch (error) {
        console.error("Analytics error:", error);
      }
  ...
  const handleBack = () => {
    // Log drop-off if user hasn't commented
    if (messages.length === 0) {
      (async () => {
        try {
          await logDropOff("Discussion", "no_comment");
        } catch (error) {
          console.error("Analytics error:", error);
        }
      })();
...
function ThreadModal({
  ...
  const handleSendReply = async () => {
    ...
    try {
      ...
      try {
        await logCommentPosted(currentQuestionId, "reply");
      } catch (error) {
        console.error("Analytics error:", error);
      }
