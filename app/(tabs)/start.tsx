...
const getNextBadgeForStreak = (streakCount: number) => {
	const badgeTiers = [
		...
		{ threshold: 30, icon: Sparkles, color: "#6B4423", name: "Insightful" },
		{
			threshold: 40,
			icon: BrainCircuit,
			color: "#8B4513",
		...
		{ threshold: 50, icon: PersonStanding, color: "#A0522D", name: "Mentor" },
		{ threshold: 60, icon: Cloudy, color: "#D2691E", name: "Visionary" },
		{ threshold: 70, icon: Biohazard, color: "#CD853F", name: "Sage" },
		{ threshold: 80, icon: Sun, color: "#B8860B", name: "Luminary" },
		{ threshold: 90, icon: Earth, color: "#DAA520", name: "Pioneer" },
		{ threshold: 100, icon: Rocket, color: "#F4C430", name: "Trailblazer" },
		{ threshold: 120, icon: Atom, color: "#FFD700", name: "Legend" },
	];
...
const HeaderBadge = ({
	...
	if (!badge) {
		"No badge found for streak count:", streakCount;
		return null;
	}
...
const StartPage = () => {
	...
	const handleVote = async (choice: "top" | "bottom") => {
		...
		try {
			// match fetchDailyQuestion's date window & selection logic
			const today = new Date();
			today.setHours(0, 0, 0, 0); // Start of today

			// NOTE: mirrors your current fetchDailyQuestion (+3 days window)
			const endOfToday = new Date(today.getTime() + 24 * 60 * 60 * 1000);

			// read user to get school
			const userDocRef = doc(firestore, "users", uid);
			const userDoc = await getDoc(userDocRef);
			const userData = userDoc.data();
			const userSchool = userData?.school;

			// query today's (3-day window) questions, same as fetchDailyQuestion
			const questionsRef = collection(firestore, "dailyQuestions");
			const q = query(
				questionsRef,
				where("date", ">=", today),
			...
			// choose doc using the same 1-or-2 doc + school matching rules
			let qDoc = qSnap.docs[0];
			if (qSnap.docs.length === 2) {
				const d1 = qSnap.docs[0].data();
				const d2 = qSnap.docs[1].data();
				if (d1.school === userSchool) {
					qDoc = qSnap.docs[0];
				} else if (d2.school === userSchool) {
					qDoc = qSnap.docs[1];
				} // else leave qDoc as qSnap.docs[0] (fallback)
			}

			// increment correct field on the chosen doc + update user doc atomically
			const field = choice === "top" ? "topCount" : "bottomCount";
			const uRef = doc(firestore, "users", uid);
			const todayDate = today.toISOString().substring(0, 10);

			const batch = writeBatch(firestore);
			batch.update(qDoc.ref, { [field]: increment(1) });
			batch.set(
				uRef,
				{
					voted: true,
			...
			// refresh UI counts from the same chosen doc
